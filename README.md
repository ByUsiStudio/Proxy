...
- [编译教程](Build.md)
- [更新日志](CHANGELOG.md)
- [安全审计与修复记录](doc/SECURITY.md) · [三端前端重构与功能说明](doc/FRONTEND.md)
...
| 自动穿透 | 把配置保存到云端，客户端带 `-deviceId -apiUser -apiPass` 启动时自动创建隧道 |
...
proxy-client -deviceId 你的设备ID -apiUser 账号 -apiPass 口令   # 开机自动创建隧道
```

> **自动穿透需要账号凭据**：云端 `GET /config/listDevice` 现在要求校验账号归属
> （此前匿名即可凭设备 ID 拿到该账号的明文口令与内网配置，属严重泄露）。
> 因此带 `-deviceId` 启动时必须同时提供 `-apiUser` / `-apiPass`（或环境变量
> `API_USER` / `API_PASS`）；未提供时客户端只打印一条提示并跳过自动穿透，不会静默失败。
